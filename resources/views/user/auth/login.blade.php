    ...
    <form method="POST" action="{{ route('login') }}" id="submitUserLoginForm">
        ...
        <div className="flex items-center justify-end mt-4">
            @if (Route::has('password.request'))
                <a className="underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800" href="{{ route('password.request') }}">
